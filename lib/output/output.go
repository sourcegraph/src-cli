...
func (o *Output) WriteMarkdown(str string, opts ...MarkdownStyleOpts) error {
	o.Lock()
	defer o.Unlock()
	return o.RenderMarkdown(o.w, str)
}

func (o *Output) RenderMarkdown(w io.Writer, str string, opts ...MarkdownStyleOpts) error {
	if !o.caps.Color {
		_, err := fmt.Fprintln(w, str)
		return err
	}

	style := glamourGetDefaultStyle(o.caps)
...
// from unexported method glamour.getDefaultStyle but specialized to style ==
// "auto" and using our detection
func glamourGetDefaultStyle(caps capabilities) glamouransi.StyleConfig {
	if !caps.Isatty {
