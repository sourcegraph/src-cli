...
// These all inspect stdout / the terminal so we hide them behind a sync.Once.
// This is because we may call detectCapabilities many times due to doing
// things like capturing output. We have observed garbled output due to
// concurrent calls to detectCapabilities.
var (
	stdoutIsAtty = sync.OnceValue(func() bool {
		return isatty.IsTerminal(os.Stdout.Fd())
	})
	stdoutWinSize = sync.OnceValues(func() (*term.Winsize, error) {
		return term.GetWinsize(os.Stdout.Fd())
	})
	isDarkBackground = sync.OnceValue(func() bool {
...
func detectCapabilities(opts OutputOpts) (caps capabilities, err error) {
	...
	if !opts.ForceDarkBackground {
		caps.DarkBackground = isDarkBackground()
	}
