...
func calcLabelWidth(bars []*ProgressBar, emojiWidth, termWidth int) int {
	labelWidth := 0
	for _, bar := range bars {
		bar.labelWidth = runewidth.StringWidth(bar.Label)
		if bar.labelWidth > labelWidth {
			labelWidth = bar.labelWidth
		}
	}

	if maxWidth := termWidth/2 - emojiWidth; (labelWidth + 2) > maxWidth {
