...
func groupFileDiffs(completeDiff []byte, defaultBranch string, groups []Group) (map[string][]byte, error) {
	...
	for _, f := range fileDiffs {
		// strip git's b/ or a/ prefix off paths
		name := strings.TrimPrefix(f.NewName, "b/")
		if name == "/dev/null" {
			name = strings.TrimPrefix(f.OrigName, "a/")
		}
		...
		for _, d := range dirs {
			// We ignore the ~ . and ./ prefixes so that we can support relative and absolute paths.
			dWithoutPrefix := strings.TrimLeft(d, "~./")
			// We have to add a trailing suffix so that we don't match files that have the same name as other directories.
			dWithDirSuffix := dWithoutPrefix + "/"
			if strings.HasPrefix(name, dWithDirSuffix) {
				matchingDir = d
			}
		}
