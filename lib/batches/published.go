...
// PushedOnly is true if the enclosed value is a string being "pushed-only".
func (p PublishedValue) PushedOnly() bool {
	if s, ok := p.Val.(string); ok {
		return s == "pushed-only"
	}
	return false
}
...
func (p *PublishedValue) Valid() bool {
	return p.True() || p.False() || p.Draft() || p.PushedOnly() || p.Nil()
}
