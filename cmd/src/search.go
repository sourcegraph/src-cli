...
// LineMatches (the preview field is not used). It uses contextLines to show
// surrounding lines of context for matches.
func applyHighlightsForFile(fileContent *string, highlights []highlight, contextLines int) string {
	var result []rune
	start := ansiColors["search-match"]
	end := ansiColors["nc"]
	...
	for _, highlight := range highlights {
		if highlight.line > maxLine {
			maxLine = highlight.line
		}
		if highlight.line < minLine {
			minLine = highlight.line
		}
	...
	contextBefore := strings.Join(lines[minLine-contextLines:minLine], "\n")
	contextAfter := strings.Join(lines[maxLine+1:maxLine+contextLines+1], "\n")
	return (contextBefore + "\n" + string(result) + "      " + contextAfter + "\n")
}
...
var searchTemplateFuncs = map[string]interface{}{
	...
	"searchHighlightMatch": func(content, query, match interface{}) string {
		...
		if strings.Contains(q, "patterntype:structural") {
			...
			c := content.(string)
			context := 3
			return applyHighlightsForFile(&c, highlights, context)
		} else {
