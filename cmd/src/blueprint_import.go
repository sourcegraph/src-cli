...
func runBlueprintImportAll(ctx context.Context, opts blueprintImportOpts, rootDir string) error {
	...
	for _, fb := range found {
		blueprintDir := rootDir
		if fb.Subdir != "" {
			blueprintDir = filepath.Join(rootDir, fb.Subdir)
		}

		fmt.Fprintf(opts.out, "--- Importing blueprint: %s", fb.Blueprint.Name)
		if fb.Subdir != "" {
			fmt.Fprintf(opts.out, " (%s)", fb.Subdir)
		}
		fmt.Fprintf(opts.out, "\n")

		summary, err := exec.Execute(ctx, fb.Blueprint, blueprintDir)
