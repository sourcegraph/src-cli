...
func init() {
	usage := fmt.Sprintf(`'src snapshot upload' uploads the files generated by 'src snapshot databases' and 'src snapshot summary' to the specified GCS bucket, for self-hosted Sourcegraph customers migrating to Sourcegraph Cloud.
...
func validateUploadInputs(bucketName, credentialsPath, fileArg string, filterSQL bool) (*uploadArgs, error) {
	...
	if bucketName == "" {
		return nil, cmderrors.Usage("-bucket required")
	}

	if credentialsPath == "" {
		return nil, cmderrors.Usage("-credentials required")
	}
...
func parseFileArg(fileArg string) ([]string, error) {
	...
	for _, part := range strings.Split(fileArg, ",") {
		...
		if !slices.Contains(listOfValidFiles, filename) {
			return nil, cmderrors.Usagef("invalid -file value %q. Valid values: %s", part, strings.Join(listOfValidFiles, ", "))
		}
