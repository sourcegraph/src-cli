...
func newLogPrinter() func(reposMap map[ActionRepo]ActionRepoStatus) {
	enqueued := map[ActionRepo]bool{}
	finished := map[ActionRepo]bool{}

	return func(reposMap map[ActionRepo]ActionRepoStatus) {
		for repo, status := range reposMap {
			_, prevEnqueued := enqueued[repo]
			_, prevFinished := finished[repo]

			switch {
			case prevEnqueued && !prevFinished && (status.Cached || !status.FinishedAt.IsZero()):
				finished[repo] = true
				...
				if status.Err != nil {
					...
					if status.Patch != (CampaignPlanPatch{}) && status.Patch.Patch != "" {
						...
						statusText = "0 files changed"
					}
				}
				// timerDuration := status.FinishedAt.Sub(status.StartedAt)

				// log.Printf("%s -> Finished. %s (%s)", repo.Name, statusText, timerDuration)

			case !prevEnqueued && !prevFinished && !status.Cached && !status.StartedAt.IsZero():
				enqueued[repo] = true

				// log.Printf("%s -> Enqueued", repo.Name)
			}
		}
	}
}
