...
func validateCurrentUser(ctx context.Context, client api.Client, out io.Writer, endpointArg string) error {
	...
	if _, err := client.NewRequest(query, nil).Do(ctx, &result); err != nil {
		if strings.HasPrefix(err.Error(), "error: 401 Unauthorized") || strings.HasPrefix(err.Error(), "error: 403 Forbidden") {
			...
			printLoginProblem(out, fmt.Sprintf("Error communicating with %s: %s", endpointArg, err))
		}
		fmt.Fprintln(out, loginAccessTokenMessage(endpointArg))
	...
	if result.CurrentUser == nil {
		...
		// a user.
		printLoginProblem(out, fmt.Sprintf("Unable to determine user on %s.", endpointArg))
		return cmderrors.ExitCode1
	}
	fmt.Fprintln(out)
