...
Examples:
  ...
  List just blueprint names and subdirs:
    	...
    	$ src blueprint list -f '{{.Subdir}}: {{.Blueprint.Name}}'
...
`
	...
	var (
		...
		formatFlag = flagSet.String("f", "{{.Blueprint.Title}}\t{{.Blueprint.Summary}}\t{{.Subdir}}", `Format for the output, using the syntax of Go package text/template. (e.g. "{{.|json}}")`)
	)
	...
	handler := func(args []string) error {
		...
		for _, fb := range found {
			if err := execTemplate(tmpl, fb); err != nil {
				return err
			}
		}
		return nil
