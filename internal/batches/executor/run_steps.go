...
type RunStepsOpts struct {
	...
	// ArtifactUploadChunkSize is the number of bytes an artifact may contain
	// before it is externalized through ArtifactUploader.
	ArtifactUploadChunkSize int64
}
...
func RunSteps(ctx context.Context, opts *RunStepsOpts) (stepResults []execution.AfterStepResult, err error) {
	...
	for i := startStep; i < len(opts.Task.Steps); i++ {
		...
		if opts.ArtifactUploader != nil {
			if err := uploadStepArtifacts(ctx, opts.ArtifactUploader, opts.ArtifactUploadChunkSize, i, &stepResult, stepOutput); err != nil {
				return stepResults, err
			}
		}
...
func executeSingleStep(
	...
	artifactUploadThreshold := opts.ArtifactUploadChunkSize
	if opts.ArtifactUploader == nil {
