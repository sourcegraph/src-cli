...
type ArtifactUploader interface {
	Upload(ctx context.Context, artifactKey string, r io.Reader) (execution.ArtifactReference, error)
}
...
func uploadStepArtifacts(ctx context.Context, uploader ArtifactUploader, threshold int64, stepIndex int, result *execution.AfterStepResult, output *stepOutput) error {
	...
	if output.stdout.shouldUpload(threshold) {
		ref, err := uploadArtifactOutput(ctx, uploader, artifactKey(stepIndex, "stdout"), output.stdout)
		if err != nil {
			return err
		}
		result.StdoutArtifact = &ref
	}

	if output.stderr.shouldUpload(threshold) {
		ref, err := uploadArtifactOutput(ctx, uploader, artifactKey(stepIndex, "stderr"), output.stderr)
		if err != nil {
			return err
		}
		result.StderrArtifact = &ref
	}

	if int64(len(result.Diff)) > threshold {
		ref, err := uploader.Upload(ctx, artifactKey(stepIndex, "diff"), bytes.NewReader(result.Diff))
		if err != nil {
			return errors.Wrap(err, "uploading diff artifact")
		}
		result.DiffArtifact = &ref
		result.Diff = nil
	}
...
func uploadArtifactOutput(ctx context.Context, uploader ArtifactUploader, key string, output *artifactOutput) (execution.ArtifactReference, error) {
	...
	if err != nil {
		return execution.ArtifactReference{}, errors.Wrapf(err, "uploading %s artifact", key)
	}
	return ref, nil
}
